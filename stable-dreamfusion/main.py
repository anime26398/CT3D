...
if __name__ == '__main__':
    ...
    parser.add_argument('--alter_indices', type=str, default=None)
    parser.add_argument('--save_inter_images', type=str, default=None)
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            if opt.save_inter_images is not None:
                clip = VideoFileClip('./'+os.path.join(opt.workspace, 'results/df_ep0050_rgb.mp4'))
                frames = clip.iter_frames()
                save_file = opt.save_inter_images
                os.makedirs(save_file, exist_ok=True)
                counter = 0
                for value in frames:
                    if counter % 5 == 0:
                        img = Image.fromarray(value).convert('RGB')
                        img.save(os.path.join(save_file, f'{counter}.png'))
                    counter += 1
        ...
        if opt.guidance == 'stable-diffusion':
            ...
            guidance = StableDiffusion(device, opt.fp16, opt.vram_O, opt.sd_version, opt.hf_key, opt.t_range, seed=opt.seed, alter_indices=opt.alter_indices)
