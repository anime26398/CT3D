...
def main():
    ...
    # print(args.indir)
    # lines = glob.glob(args.indir + '/*00000_input.jpg')
    # img_names = [line.rstrip('\n').split('/')[-1].replace('_input.jpg','') for line in lines]

    # part_len = len(img_names)//4
    # if args.id == 4:
    #     img_names = img_names[(args.id-1)*part_len:]
    # else:
    #     img_names = img_names[(args.id-1)*part_len:(args.id)*part_len]
    # print(len(img_names))
    print(args.indir)
    img_names = glob.glob(args.indir + '/*.png')

    # edits = ['make in sunset',
    #          'make it autumn',
    #          'make it midnight',
    #          'make it storm',
    #          'make it natural']
    ...
    for i in tqdm.tqdm(range(len(img_names))):
        ...
        # img_path = os.path.join(args.indir, img_name+'_input.jpg')
        # save_path = args.indir + '_variants'
        ...
        with torch.no_grad(), autocast("cuda"), model.ema_scope():
            ...
            for edit in edits:
                ...
                edited_image.save(os.path.join(args.output, img_name))
                cnt += 1
